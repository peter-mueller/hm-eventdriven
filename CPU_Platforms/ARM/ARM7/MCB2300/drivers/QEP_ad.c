...
__irq  void ADC_IRQHandler(void) {
 ...
 // AD_last = (AD0DR0 >> 6) & 0x3FF;      /* Read Conversion Result             */
  ...
  AD_Event *ope = Q_NEW(AD_Event, AD_READY_SIG);
   
   ope->new_conversion_value = ad.new_conversion_value = (AD0DR0 >> 6) & 0x3FF;      /* Read Conversion Result             */
#ifdef XXX
	__disable_irq();
    QActive_postFIFO( Blinky, (QEvent *)ope);    
